    ...
    _refresh: function(state,from,to) {
      ...
      this.items.each(function(i) {
        var side = (i == to && from-to < 0 ) ||  i-to > 0 ? 'left' : 'right',
            ...
            zclass = self.items.length + (side == "left" ? to-i : i-to);

        $(this).attr('data', zclass)

        if (vendorPrefix == 'ms' || vendorPrefix == "") {
          css["filter"] = "progid:DXImageTransform.Microsoft.Matrix(sizingMethod='auto expand', M11=0.9, M12=0, M21=" + (mod * (side == 'right' ? -0.2 : 0.2)) + ", M22=0.9";
          css[self.props[2]] = ( (-i * (self.itemSize/2)) + (side == 'right'? -self.itemSize/2 : self.itemSize/2) * mod );
          if (i == self.current) {
            css.width = self.itemWidth * (.7+((1-mod)*0.5));
          ...
          css[vendorPrefix + 'Transform'] = 'matrix(1,'+(mod * (side == 'right' ? -0.2 : 0.2))+',0,1,0,0) scale('+(.7+((1-mod)*0.5))+')';
          css[self.props[2]] = ( (-i * (self.itemSize/2)) + (side == 'right'? -self.itemSize/2 : self.itemSize/2) * mod );
        }
        $(this).css(css);
      });
